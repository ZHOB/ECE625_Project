...
fold_ids      <- rep(seq(kfolds), 
                         ceiling(nrow(train) / kfolds))
fold_ids      <- fold_ids[1:nrow(train)]
fold_ids      <- sample(fold_ids, length(fold_ids))

CV_error_vec  <- vector(length = kfolds, mode = "numeric")

#Create 10 equally size folds
folds <- cut(seq(1,nrow(house)),breaks=10,labels=FALSE)
...
for (k in 1:kfolds){
  	...
  	testData <- house[testIndexes, ]
  	trainData <- house[-testIndexes, ]
  	  #Use the test and train data partitions however you desire...

	    lda_model <- lda(increase~#landuse_description
					#+market_building_class
					property_type #***useful feature
					#+effective_build_year #not useful
					+net_area # *
